...
function validateUrl(rawUrl) {
    ...
    try {
        ...
        throw { status: 400, message: 'The provided URL is invalid.' };
    }
...
async function handleAudit(req, res) {
    let targetUrl;
    try {
        targetUrl = validateUrl(req.body?.url);
    } catch (err) {
        return res.status(err.status || 400).json({ detail: err.message || 'Invalid URL' });
    }
    ...
    try {
        ...
        if (!isHtmlType && !isHtmlBody && contentType) {
            return res.status(415).json({
                detail: `Expected an HTML document but received Content-Type '${contentType || 'unknown'}'.`,
            });
        }
        if (Buffer.byteLength(htmlText, 'utf8') > 5 * 1024 * 1024) {
            return res.status(500).json({
                detail: 'Target response size exceeds maximum allowed limit (5MB).',
            });
        }
        ...
        if (err.name === 'AbortError') {
            return res.status(408).json({
                detail: `Request to ${targetUrl} timed out after 10s.`,
            });
        }
        return res.status(500).json({
            detail: `Could not reach ${targetUrl}: ${err.message || 'Upstream fetch error'}`,
        });
    }
}
